...
class storage_iterator final {
    static_assert(!internal::is_polymorphic_component_container_v<typename Container::value_type>);
...
class storage_iterator<Container, std::enable_if_t<internal::is_polymorphic_component_container_v<std::remove_pointer_t<typename Container::value_type>>>> final {
    static_assert(!internal::is_polymorphic_component_container_v<typename Container::value_type>);

    friend storage_iterator<const Container>;

public:
    using value_type = internal::unwrap_polymorphic_component_container_t<std::remove_pointer_t<typename Container::value_type>>;
...
private:
    ...
    using iterator_traits = std::iterator_traits<std::conditional_t<
        std::is_const_v<Container>,
        typename alloc_traits::template rebind_traits<value_type>::const_pointer,
        typename alloc_traits::template rebind_traits<value_type>::pointer>>;
...
public:
    ...
    /*! @copydoc get */
    template<typename T>
    [[nodiscard]] const T &get_as(const entity_type entt) const ENTT_NOEXCEPT {
    ...
    /*! @copydoc get */
    template<typename T>
    [[nodiscard]] T &get_as(const entity_type entt) ENTT_NOEXCEPT {
        return const_cast<T &>(std::as_const(*this).template get_as<T>(entt));
    }
    ...
    /*! @copydoc get_as_tuple */
    template<typename T>
    [[nodiscard]] std::tuple<const T &> get_as_tuple_as(const entity_type entt) const ENTT_NOEXCEPT {
        return std::forward_as_tuple(get_as<T>(entt));
    }

    /*! @copydoc get_as_tuple */
    template<typename T>
    [[nodiscard]] std::tuple<T &> get_as_tuple_as(const entity_type entt) ENTT_NOEXCEPT {
        return std::forward_as_tuple(get_as<T>(entt));
    }
    ...
    /*! @copydoc emplace */
    template<typename... Args>
    value_type &emplace(entt::basic_registry<entity_type>&, const entity_type entt, Args &&...args) {
        return emplace(entt, std::forward<Args>(args)...);
    }
...
class basic_storage<Entity, Type, Allocator, std::enable_if_t<is_polymorphic_component_v<Type>>>
    : public basic_sparse_set<Entity, typename std::allocator_traits<Allocator>::template rebind_alloc<Entity>> {
    static_assert(std::is_same_v<typename std::allocator_traits<Allocator>::value_type, Type>);

    using comp_traits = component_traits<Type>;
    using component_container_type = internal::polymorphic_component_container<Entity, Type, Allocator>;
    using alloc_traits = typename std::allocator_traits<Allocator>::template rebind_traits<component_container_type>;
    using underlying_type = basic_sparse_set<Entity, typename alloc_traits::template rebind_alloc<Entity>>;
    using container_type = std::vector<typename alloc_traits::pointer, typename alloc_traits::template rebind_alloc<typename alloc_traits::pointer>>;
...
private:
    ...
    void swap_at(const std::size_t lhs, const std::size_t rhs) final {
        ENTT_ASSERT(false, "swap_at should never be called for hierarchical components");
    }

    void move_element(const std::size_t from, const std::size_t to) final {
        ENTT_ASSERT(false, "move_element should never be called for hierarchical components");
    }

protected:
    /**
     * @brief Erase
     * s elements from a storage.
     * @param first An iterator to the first element to erase.
     * @param last An iterator past the last element to erase.
     */
    void swap_and_pop(typename underlying_type::basic_iterator first, typename underlying_type::basic_iterator last) override {
        ENTT_ASSERT(false, "swap_and_pop should never be called for hierarchical components");
    }
    ...
    typename underlying_type::basic_iterator try_emplace([[maybe_unused]] const Entity entt, const bool force_back, const void *value) override {
        if(value) {
            ...
            if constexpr(std::is_default_constructible_v<component_container_type>) {
                return emplace_element(entt, force_back);
            } else {
                return base_type::end();
            }
...
public:
     ...
     */
    [[nodiscard]] const value_type &get(const entity_type entt) const ENTT_NOEXCEPT {
        return element_at(base_type::index(entt)).ref();
    }
    ...
    [[nodiscard]] decltype(auto) get_as(const entity_type entt) ENTT_NOEXCEPT {
        ...
        if constexpr(internal::is_every_wrap_v<T>) {
            if constexpr(internal::is_const_every_wrap_v<T>) {
                ...
                return element_at(base_type::index(entt)).each();
            }
        } else {
            return get(entt);
        }
...
public:
    ...
    /*! @copydoc get_as_tuple */
    template<typename T>
    [[nodiscard]] decltype(auto) get_as_tuple_as(const entity_type entt) const ENTT_NOEXCEPT {
        if constexpr(internal::is_every_wrap_v<T>) {
            return std::make_tuple(get_as<T>(entt));
        } else {
            return std::forward_as_tuple(get_as<T>(entt));
        }
    }
    ...
    /*! @copydoc get_as_tuple */
    template<typename T>
    [[nodiscard]] decltype(auto) get_as_tuple_as(const entity_type entt) ENTT_NOEXCEPT {
        if constexpr(internal::is_every_wrap_v<T>) {
            return std::make_tuple(get_as<T>(entt));
        } else {
            return std::forward_as_tuple(get_as<T>(entt));
    ...
    template<typename... Args>
    value_type &emplace(const entity_type entt, Args &&...args) = delete;
     ...
     */
    template<typename... Args>
    value_type &emplace(basic_registry<entity_type>& reg, const entity_type entt, Args &&...args) {
        if (base_type::contains(entt)) {
            auto& elem = element_at(base_type::index(entt));
            elem.construct_value(reg, entt, std::forward<Args>(args)...);
            return elem.ref();
        } else {
            typename underlying_type::iterator it;
            if constexpr(std::is_aggregate_v<component_container_type>) {
                it = emplace_element(entt, false, Type{std::forward<Args>(args)...});
            } else {
                it = emplace_element(entt, false, std::forward<Args>(args)...);
            }
            component_container_type& hierarchical = element_at(static_cast<size_type>(it.index()));
            hierarchical.emplace_hierarchy_after_construct(reg, entt);
            return hierarchical.ref();
        }
    }
    ...
    value_type &patch(const entity_type entt, Func &&...func) {
        const auto idx = base_type::index(entt);
        value_type &elem = element_at(idx);
        (std::forward<Func>(func)(elem), ...);
        return elem;
    }
    ...
    void erase(const entity_type entt) = delete;

    void erase(basic_registry<entity_type>& reg, const entity_type entt) {
        component_container_type& comp_container = element_at(base_type::index(entt));
        if (comp_container.destroy_all_refs(reg, entt) || comp_container.destroy_value(reg, entt)) {
            base_type::remove(entt);
        }
    }

    bool remove(const entity_type entt) = delete;

    bool remove(basic_registry<entity_type>& reg, const entity_type entt) {
        return base_type::contains(entt) && (erase(reg, entt), true);
    }
...
private:
    void emplace_ref(const entity_type entt, Type& r, void* deleter) {
        ...
        if (base_type::contains(entt)) {
            ...
            emplace_element(entt, false, ref);
        }
    }

    void erase_value(basic_registry<entity_type>& reg, const entity_type entt) {
        if (element_at(base_type::index(entt)).destroy_value(reg, entt)) {
...
private:
    compressed_pair<container_type, typename alloc_traits::allocator_type> packed;
};
...
public:
    ...
    /** @copydoc get */
    template<typename T>
    void get_as ([[maybe_unused]] const entity_type entt) const ENTT_NOEXCEPT {
        ENTT_ASSERT(base_type::contains(entt), "Storage does not contain entity");
    }
    ...
    /** @copydoc get_as_tuple */
    template<typename T>
    std::tuple<> get_as_tuple_as ([[maybe_unused]] const entity_type entt) const ENTT_NOEXCEPT {
        ENTT_ASSERT(base_type::contains(entt), "Storage does not contain entity");
        return std::tuple{};
    }
    ...
    template<typename... Args>
    void emplace(basic_registry<entity_type>&, const entity_type entt, Args &&...) {
        base_type::try_emplace(entt, false);
    }
